...
{
#endif
    // static const char *TAG_tmccb = "tmc_USB_cb";
    ...
    {
        (void)interface_id;
        // Debes llamar a esto para que el host pueda enviar datos
        tud_usbtmc_start_bus_read();
    }
    ...
    // 7. Callback para el STATUS BYTE (IEEE 488)
    uint8_t tud_usbtmc_get_stb_cb(uint8_t *tmcResult)
    {
        *tmcResult = USBTMC_STATUS_SUCCESS;
        return 0; // Status Byte con bit MAV = 0 (no hay mensajes disponibles)
    }

    // 8. Callback para el TRIGGER (IEEE 488)
